	...
	autoComplete: function (options) {
		...
		function AutoComplete($ele, options) {
			...
			this.defaults = {
				...
				autoClass: {
					wrap: "autoComplete",
					tagsWrap: "autoComplete-tags",
					tag: "tag",
					input: "autoComplete-input"
				},
				//ajax获取数据的url
				serviceUrl: $ele.attr("data-url") || "",
				//本地的数据,type:array
				lookup: null,
				//显示的数据条数
				lookupLimit: $ele.attr("data-limit") || 10,
				//可选定的最大数量
				maxNumber: $ele.attr("data-max") || 3,
				//满足字符长度即开始搜索
				minChars: $ele.attr("data-min") || 1,
				//ajax参数
				paramName : "",
				zIndex: 9999,
				//补全控件高度
				maxHeight: 200,
				//选择时触发,function(suggestions){}
				onSelect: noop,
				//搜索开始时触发,function (query) {}
				onSearchStart: noop,
				//搜索完成时触发,function (query, suggestions) {}
				onSearchComplete: noop,
				//渲染html前触发,可用于自定义样式,function (container) {}
				beforeRender: noop,
				//设置显示的信息,默认是value,如"id,name,age",显示的结果为value id name age
				formatResult: $ele.attr("data-format") || "",
				//是否显示没有补全时的信息
				showNoSuggestionNotice: $ele.attr("data-showNotice") || false,
				//编辑没有补全时的信息
				noSuggestionNotice: $ele.attr("data-notice") || "没有匹配结果"
			};
		...
		AutoComplete.prototype = {
			...
			html: function () {
				var that = this,
					autoClass = that.options.autoClass,
					width = that.$ele.width();
				that.$ele.after(
					'<div class="' + autoClass.wrap + '" style="width:' + width + 'px">' +
					'<div class="' + autoClass.tagsWrap + '"></div>' +
					'<input type="text" class="' + autoClass.input + '">' +
					'</div>'
				).hide();
			},
			bindEvents: function () {
				var that = this,
					autoClass = that.options.autoClass;
				var $input = $("." + autoClass.input),
					$autoWrap = $("." + autoClass.wrap);
				...
				$autoWrap
					...
					.on("click", ".cancel-tag", function () {
						$(this).closest(".tag").remove();
					})
					.on("blur", "." + autoClass.input, function () {
						$autoWrap.removeClass("autoComplete-active");
					});
				this.autoInit();
			},
			autoInit: function () {
				...
				$('.autoComplete-input').autocomplete({
					...
					lookupLimit: options.lookupLimit,
					...
					onSelect: function (suggestion) {
						...
						var tagWrap = "." + autoClass.tagsWrap,
							tag = "." + autoClass.tag,
							currentLength = $(tagWrap).find(tag).length;
						if (currentLength < options.maxNumber) {
							var tagClass = "." + options.autoClass.tagsWrap;
							$(tagClass).append(
								'<span class="' + autoClass.tag + '"' +
								(function () {
									var data = "";
									options.formatResult.split(",").map(function (i) {
										data += "data-" + i + "='" + suggestion[i] + "'";
									});
									return data;
								})() +
								'>' + suggestion.nickname + '<i class="fa fa-times cancel-tag"></i></span>'
							);
						} else {
							console.warn("已超过控件允许的最大数量");
							notify && notify.warn("已超过控件允许的最大数量");
						}

						$(this).val("").hide();
					},
					showNoSuggestionNotice: options.showNoSuggestionNotice,
					...
					formatResult: function (suggestion, currentValue) {
						...
						var result = " ";
						options.formatResult && options.formatResult.split(",").map(function (i) {
							result += suggestion[i] + " ";
						});
						...
						return suggestion.nickname
								...
								.replace(/>/g, '&gt;')
								.replace(/"/g, '&quot;')
								.replace(/&lt;(\/?strong)&gt;/g, '<$1>') + result;
					}
				});
			},
			//得到所选中的值,没有补全的话返回所输入的值,有补全的话返回所选中项的信息,[object1,object2...]
			getValue: function () {
				var that = this,
					autoClass = that.options.autoClass,
					inputClass = "." + autoClass.input,
					tagWrap = "." + autoClass.tagsWrap,
					tag = "." + autoClass.tag,
					tags = $(tagWrap).find(tag);
				if (tags.length == 0) {
